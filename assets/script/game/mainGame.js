...
localStorage.setItem("FM : setting key",localStorage.getItem("FM : setting key"))

localStorage.setItem("FM : custom icon bumping",localStorage.getItem("FM : custom icon bumping"))

// detect localStorage value

if (localStorage.getItem("FM : setting key") !== "null") {
  if (localStorage.getItem("FM : setting key").length >= 1) {
    game.settings.key = [localStorage.getItem("FM : setting key")[0],localStorage.getItem("FM : setting key")[1],localStorage.getItem("FM : setting key")[2],localStorage.getItem("FM : setting key")[3]]
  }
}

if (localStorage.getItem("FM : custom icon bumping") == "null") {
  localStorage.setItem("FM : custom icon bumping","default")
}

if (game.settings.devMods == false && localStorage.getItem("FM : botplay") !== undefined) {
  game.settings.botplay = localStorage.getItem("FM : botplay");
}
...
function loadStage(data) {
  if (data.stage["is animated ?"] == false) {
    game.stage.BG = document.createElement("img");
    document.getElementById("GAME").appendChild(game.stage.BG);
    
    game.stage.BG.id = "main BG"
    game.stage.BG.style.position = "absolute";
    game.stage.BG.src = "mods/" + game.modsSelect + "/" + data.stage.src + ".png";
    game.stage.BG.onerror = function() {
      PreError("fail to load the background", false, false)
    }
    game.stage.BG.style.left = data.stage.position[0] + "px";
    game.stage.BG.style.top = data.stage.position[1] + "px";
    game.stage.BG.style.scale = data.stage.scale;
    game.stage.BG.style.zIndex = -10;
  } else {
    makeLocalAnim("main BG","mods/" + game.modsSelect + "/" + data.stage.src, data.stage.length, data.stage.position[0], data.stage.position[1], data.stage.scale,"GAME", fpsToMS(data.stage.fps),true, 0, 0, false, data.stage["Padding Length ?"])
...
function onPreLoad() { if (antiLoop == 0 ) {
  ...
  makeNote(253,"player"); // default is 153
  loadPauseMenu();
...
function camMove(x,y) { // took 1 hours, just for that 
  if (game.cam.type == 1) {
      ...
      FMS_makeCSSanim("GAME", game.cam.speed * 0.3, "ease-out", 1, [
