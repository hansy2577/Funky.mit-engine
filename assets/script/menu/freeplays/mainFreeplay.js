...
if (localStorage.getItem("mods choise") !== null) {
  menu.modsChoise = Number.parseInt(localStorage.getItem("mods choise"));
}
...
function MakeSongElement(name,pos,altName) {
  ...
  var loop = setInterval(() => {
    if (menu.tuleDelete) {
      disableMove = true;
      var move = FMS_makeCSSanim(song.id, Math.random() * 1.5, "ease-out", "1", [
...
function changeModsSelect() {
  ...
  if (localStorage.getItem("mods choise") >= menu.modsList.mods.length - 1) {
    menu.modsChoise = 0;
  }
  localStorage.setItem("mods choise",menu.modsChoise)
  document.getElementById("modsSelect").innerText = "mods: "+menu.modsList.mods[menu.modsChoise]
